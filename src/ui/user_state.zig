...
pub const State = struct {
    ...
    /// Timeline-Abschnitt im Explorer aufgeklappt (VS Code: anfangs eingeklappt)
...
pub fn parse(text: []const u8) State {
    ...
    while (lines.next()) |raw| {
        ...
        } else if (std.mem.eql(u8, key, "timeline_expanded")) {
            st.timeline_expanded = std.mem.eql(u8, value, "true") or std.mem.eql(u8, value, "1");
        }
    }
    return st;
}
...
pub fn format(alloc: std.mem.Allocator, st: State) ![]u8 {
    return std.fmt.allocPrint(alloc, "# zid state\nsidebar_width={d}\nshow_hidden={s}\ntheme={s}\nfont_size={d}\nautosave={s}\nminimap={s}\nwhitespace={s}\nindent_guides={s}\nword_wrap={s}\ntimeline_expanded={s}\n", .{
        ...
        if (st.word_wrap) "true" else "false",                   if (st.timeline_expanded) "true" else "false",
    });
}
...
test "timeline_expanded: Standard eingeklappt wie VS Code, gemerkt" {
    try testing.expect(!parse("").timeline_expanded);
    try testing.expect(parse("timeline_expanded=true\n").timeline_expanded);
    const text = try format(testing.allocator, .{ .timeline_expanded = true });
    defer testing.allocator.free(text);
    try testing.expect(parse(text).timeline_expanded);
}
