...
//!
//! Multi-line Text-Editor mit Cursor, Selection, Scrollbar und Context Menu.
//! Basierend auf TextAreaState - ohne Line Numbers und Syntax Highlighting.
...
pub const TextAreaState = struct {
    ...
    /// Flag: ist dies eine TextArea (kein Editor)?
    ...
    fn renderContextMenu(self: *Self, arena: std.mem.Allocator, mouse_pressed: bool) void {
        ...
        var item_count: f32 = 3; // Cut, Copy, Paste
        const path = self.buffer.get_file_path();
        const is_md = std.mem.endsWith(u8, path, ".md") and !self.is_textarea;
...
test "setText: CRLF wird zu LF normalisiert" {
    ...
    defer buffer.deinit();
    var ed = TextAreaState.init(allocator, buffer);
    defer ed.deinit();
