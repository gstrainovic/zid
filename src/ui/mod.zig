...
pub const UI = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, config: UIConfig, default_file_path: ?[]const u8) !Self {
        ...
        if (default_file_path) |path| {
            const file_content = std.fs.cwd().readFileAlloc(allocator, path, 64 * 1024 * 1024) catch |err| {
                log.err("Failed to load default file '{s}': {any}", .{ path, err });
                return Self{
                    .allocator = allocator, .config = config, .theme = Theme.dark(),
                    .clay_memory = clay_memory, .initialized = false, .anim_manager = AnimationManager.init(allocator),
                    .frame_arena = std.heap.ArenaAllocator.init(allocator), .code_editor = code_editor,
                    .text_renderer = null, .tab_bar = tab_bar, .file_explorer = file_explorer,
                    .show_file_explorer = true, .current_directory = null, .open_images = std.StringHashMap(*anyopaque).init(allocator),
                    .open_pdfs = std.StringHashMap(*anyopaque).init(allocator), .open_markdown_views = std.StringHashMap(*markdown_view_mod.MarkdownView).init(allocator),
                    .image_renderer = null, .pending_tab_switch = null, .mouse_pressed_this_frame = false,
                    .mouse_x = 0, .mouse_y = 0, .is_mouse_down = false,
                };
            };
    ...
    pub fn handleKeyPress(self: *Self, key: @import("wio").Button) void {
        if (self.getActiveTerminal()) |term| {
            var data: ?[]const u8 = switch (key) {
                ...
                .home => "\x1b[H", .end => "\x1b[F", .delete => "\x1b[3~", .page_up => "\x1b[5~", .page_down => "\x1b[6~",
                else => null,
            };
            if (self.code_editor.mods.ctrl) {
                data = switch (key) {
                    ...
                    .u => "\x15", .v => "\x16", .w => "\x17", .x => "\x18", .y => "\x19", .z => "\x1A",
                    else => data,
                };
            }
            if (data) |d| term.sendInput(d) catch {};
            return;
        }
    ...
    pub fn setShiftState(self: *Self, pressed: bool) void { self.code_editor.setShiftState(pressed); }
    pub fn setCtrlState(self: *Self, pressed: bool) void { self.code_editor.setCtrlState(pressed); }
    pub fn setAltState(self: *Self, pressed: bool) void { self.code_editor.setAltState(pressed); }
    ...
    pub fn renderExample(self: *Self, image_data: ?*const anyopaque) []clay.RenderCommand {
        ...
        clay.UI()(.{
            ...
            clay.UI()(.{
                ...
                .layout = .{ .sizing = .{ .w = .grow, .h = .fixed(56) }, .direction = .left_to_right, .child_alignment = .{ .x = .left, .y = .center }, .padding = .{ .left = 16, .right = 16 }, .child_gap = 16 },
                .background_color = t.surface, .border = .{ .width = .{ .bottom = 1 }, .color = t.border },
            })({
                if (image_data) |ptr| {
                    clay.UI()(.{ .id = clay.ElementId.ID("Logo"), .layout = .{ .sizing = .{ .w = .fixed(32), .h = .fixed(32) } }, .image = .{ .image_data = ptr } })({});
                }
                clay.text("VULKAN-ED", .{ .font_size = 24, .color = t.text });
            });

            // Main Content
            clay.UI()(.{ .id = clay.ElementId.ID("MainContent"), .layout = .{ .sizing = .grow, .direction = .left_to_right }, .background_color = t.bg })({
                const splitter_id = clay.ElementId.ID("ExplorerSplitter");
                if (self.show_file_explorer) {
                    if (self.file_explorer.is_resizing) {
                        if (!self.is_mouse_down) { self.file_explorer.is_resizing = false; }
                        else {
                    ...
                    } else if (clay.pointerOver(splitter_id) and self.mouse_pressed_this_frame) { self.file_explorer.is_resizing = true; }
                    file_explorer_mod.renderFileExplorer(self.frame_arena.allocator(), &self.file_explorer, t, self.mouse_pressed_this_frame);
                    self.file_explorer.processPendingToggle();
                    clay.UI()(.{ .id = splitter_id, .layout = .{ .sizing = .{ .w = .fixed(4), .h = .grow } }, .background_color = if (self.file_explorer.is_resizing) t.primary else if (clay.pointerOver(splitter_id)) t.secondary else t.border })({});
                }

                clay.UI()(.{ .id = clay.ElementId.ID("EditorArea"), .layout = .{ .sizing = .grow, .direction = .top_to_bottom }, .background_color = t.bg })({
                    tab_bar_mod.renderTabBar(self.frame_arena.allocator(), &self.tab_bar, t, self.mouse_pressed_this_frame);
                    ...
                    if (self.tab_bar.active_index) |idx| {
                        if (idx < self.tab_bar.tabs.items.len) {
                            ...
                            if (tab.kind == .image) {
                                image_view_mod.ImageViewState.render(self.frame_arena.allocator(), tab.path, t, &self.open_images);
                                special_active = true;
                            } else if (tab.kind == .pdf) {
                                if (self.open_pdfs.get(tab.path)) |handler_ptr| {
                                    const handler: *PdfHandler = @ptrCast(@alignCast(handler_ptr));
                                    if (PdfViewState.render(handler, self.open_images.get(tab.path), t, self.mouse_pressed_this_frame)) |delta| {
                    ...
                    if (!special_active) self.code_editor.render(self.frame_arena.allocator());
                });
            });
        });
    ...
    pub fn getDesiredCursor(self: *Self) wio.Cursor {
        if (self.file_explorer.is_resizing or clay.pointerOver(clay.ElementId.ID("ExplorerSplitter"))) return .size_ew;
        return self.code_editor.desired_cursor;
    }

    pub fn isTerminalActive(self: *Self) bool {
        if (self.tab_bar.active_index) |idx| if (idx < self.tab_bar.tabs.items.len) return self.tab_bar.tabs.items[idx].kind == .terminal;
        return false;
    }
    ...
    pub fn getActiveTerminal(self: *Self) ?*@import("../terminal/terminal_instance.zig").TerminalInstance {
        if (self.tab_bar.active_index) |idx| {
            if (idx < self.tab_bar.tabs.items.len) {
                const tab = self.tab_bar.tabs.items[idx];
                if (tab.kind == .terminal) return self.tab_bar.terminal_instances.get(tab.path);
            }
        }
        return null;
    }
    ...
    fn renderTerminalContent(self: *Self, path: []const u8, t: Theme) void {
        ...
        clay.UI()(.{ .id = clay.ElementId.ID("terminal_outer"), .layout = .{ .sizing = .grow, .direction = .left_to_right, .padding = .all(8) }, .background_color = .{ 30, 30, 30, 255 } })({
            clay.UI()(.{ .id = clay.ElementId.ID("terminal_content_clip"), .layout = .{ .sizing = .grow }, .clip = .{ .vertical = true, .horizontal = true } })({
                clay.UI()(.{ .id = clay.ElementId.ID("terminal_content"), .layout = .{ .sizing = .{ .w = .grow, .h = .fit }, .direction = .top_to_bottom } })({
                    ...
                    while (i < end_line) : (i += 1) {
                        ...
                        clay.UI()(.{ .id = clay.ElementId.IDI("term_line", @intCast(i)), .layout = .{ .sizing = .{ .w = .grow, .h = .fixed(line_height) } } })({
                            clay.text(line_text, .{ .font_size = 16, .color = .{ 240, 240, 240, 255 } });
                        });
                    }
                });
            });
